...
### Use `mcp__plugin_context-mode_context-mode__ctx_execute` instead of Bash when:
...
### Use `mcp__plugin_context-mode_context-mode__ctx_batch_execute` when:
- Running multiple independent commands in one call
  (e.g., tsc + bun test + biome check simultaneously)

### Use `mcp__plugin_context-mode_context-mode__ctx_execute_file` when:
- Processing a file's contents without reading raw text into context

### Use `mcp__plugin_context-mode_context-mode__ctx_fetch_and_index` + `mcp__plugin_context-mode_context-mode__ctx_search` when:
